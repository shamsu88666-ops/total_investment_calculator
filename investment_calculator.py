...
# --- CUSTOM CSS (FORCED DARK THEME - WORKS ON ALL DEVICES) ---
st.markdown("""
    <style>
...
def calculate_lumpsum_future_value(lumpsum_amount, annual_return, years):
    ...
    # Convert annual nominal rate to effective annual compounding
    annual_rate = annual_return / 100
...
st.markdown("""
    <p style='text-align: center; color: #9CA3AF; font-size: 16px;'>
        Calculate combined returns from SIP and Lumpsum investments
    </p>
    """, unsafe_allow_html=True)
...
with col1:
    ...
    sip_amount = st.number_input(
        ...
        value=5000, 
        step=500,
        help="Enter monthly SIP investment amount"
    )
...
with col2:
    ...
    lumpsum_amount = st.number_input(
        ...
        value=50000, 
        step=1000,
        help="Enter one-time lumpsum investment amount"
    )
...
with col3:
    investment_years = st.number_input(
        ...
        value=10,
        step=1,
        help="Number of years you plan to stay invested"
    )
...
with col4:
    expected_return = st.number_input(
        ...
        value=12.0,
        step=0.5,
        help="Expected annual rate of return (effective rate)"
    )
    ...
    with col_res1:
        if sip_amount > 0:
            st.markdown("""
                ...
                """.format(
                    format_currency(sip_amount * 12 * investment_years),
                    format_currency(sip_future),
                    format_currency(sip_future - (sip_amount * 12 * investment_years))
                ), unsafe_allow_html=True)
        else:
    ...
    with col_res2:
        if lumpsum_amount > 0:
            st.markdown("""
                ...
                """.format(
                    format_currency(lumpsum_amount),
                    format_currency(lumpsum_future),
                    format_currency(lumpsum_future - lumpsum_amount)
                ), unsafe_allow_html=True)
        else:
    ...
    if sip_amount > 0 or lumpsum_amount > 0:
        ...
        with col_total1:
            st.markdown("""
                ...
                """.format(format_currency(total_investment)), unsafe_allow_html=True)
        ...
        with col_total2:
            st.markdown("""
                ...
                """.format(format_currency(total_return)), unsafe_allow_html=True)
        ...
        with col_total3:
            st.markdown("""
                <div class='metric-box' style='background-color: #1A2233; border-left: 5px solid #22C55E;'>
                    ...
                    <div class='metric-value' style='color: #22C55E; font-size: 32px;'>{}</div>
                </div>
                """.format(format_currency(total_future)), unsafe_allow_html=True)
...
with st.sidebar:
    ...
    st.markdown("""
        This calculator helps you plan your investments by showing:
        - Individual returns from SIP and Lumpsum
    ...
    st.markdown("""
        ...
        - Clear visibility of all text
        - Mobile responsive design
    """)
    
    st.markdown("### 📊 Assumptions")
    st.markdown("""
        - SIP investments made at beginning of month
        - Returns compounded monthly using **effective rate**
